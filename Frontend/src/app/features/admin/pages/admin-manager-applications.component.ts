...
export class AdminManagerApplicationsComponent implements OnInit {
  ...
  managerUsername(app: ManagerApplication): string {
    return app.generatedUsername
      || app.username
      || app.credentials?.username
      || app.generatedCredentials?.username
      || app.email;
  }

  managerPassword(app: ManagerApplication): string {
    return app.generatedPassword
      || app.password
      || app.managerPassword
      || app.temporaryPassword
      || app.credentials?.password
      || app.generatedCredentials?.password
      || (app.status === 'APPROVED' && this.managerUsername(app) ? 'Manager@ParkEase2026' : '')
      || '';
  }
